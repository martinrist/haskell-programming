module HaskellProgramming.Chapter12.Exercises.MaybeSpec where

import HaskellProgramming.Chapter12.Exercises.Maybe
import Test.Hspec

testIsJust :: Spec
testIsJust =
    context "isJust" $ do
        it "Returns True for Just x" $ isJust (Just (1 :: Int)) `shouldBe` True
...
testIsNothing =
    context "isNothing" $ do
        ...
        it "Returns False for Just x" $ isNothing (Just (1 :: Int)) `shouldBe` False

testMayybee :: Spec
testMayybee =
    context "mayybee" $ do
        it "Returns default value if called with Nothing" $
            mayybee 0 (+ 1) Nothing `shouldBe` (0 :: Int)
        it "Applies function to x if called with Just x" $
            mayybee 0 (+ 1) (Just 1) `shouldBe` (2 :: Int)
...
testListToMaybe =
    context "listToMaybe" $ do
        it "Returns Just the head of the list for non-empty list" $
            listToMaybe [1, 2, 3] `shouldBe` Just (1 :: Int)
...
testMaybeToList =
    context "maybeToList" $ do
        it "Returns single element list for Just x" $
            maybeToList (Just 1) `shouldBe` [1 :: Int]
...
testCatMaybes =
    context "catMaybes" $ do
        ...
        it "Removes single Nothing at start" $
            catMaybes [Nothing, Just 2, Just 3] `shouldBe` [2 :: Int, 3]
        it "Removes single Nothing in middle" $
            catMaybes [Just 1, Nothing, Just 3] `shouldBe` [1 :: Int, 3]
        it "Removes single Nothing at end" $
            catMaybes [Just 1, Just 2, Nothing] `shouldBe` [1 :: Int, 2]
        it "Removes multiple Nothings" $
            catMaybes [Just 1, Nothing, Nothing, Just 4] `shouldBe` [1 :: Int, 4]

testFlipMaybe :: Spec
testFlipMaybe =
    context "flipMaybe" $ do
        it "Returns Nothing if any element is Nothing" $
            flipMaybe [Just (1 :: Int), Nothing] `shouldBe` Nothing
        it "Returns Just [a] if all elements are Justs" $
            flipMaybe [Just (1 :: Int), Just 2, Just 3] `shouldBe` Just [1, 2, 3]
